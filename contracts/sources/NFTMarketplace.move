...
address 0x028afc12e78df508b1c97f89a3a3e9a9f9480d648f7b17149caf928cef84adfe {
...
// Get Total NFTs in the Marketplace
#[view]
...
// Get Total NFTs for Sale
#[view]
public fun get_total_nfts_for_sale(marketplace_addr: address): u64 acquires Marketplace {
    let marketplace = borrow_global<Marketplace>(marketplace_addr);
    let nfts = &marketplace.nfts;
    let nfts_len = vector::length(nfts);
    let count = 0u64; // No "mut" keyword needed.

    let i = 0;
    while (i < nfts_len) {
        let nft = vector::borrow(nfts, i);
        if (nft.for_sale) {
            count = count + 1; // Simply increment the count.
        };
        i = i + 1;
    };
    count
...
// Get Total NFTs by Rarity
#[view]
public fun get_total_nfts_by_rarity(marketplace_addr: address, rarity: u8): u64 acquires Marketplace {
    let marketplace = borrow_global<Marketplace>(marketplace_addr);
    let nfts = &marketplace.nfts;
    let nfts_len = vector::length(nfts);
    let count = 0u64;
...
// Get Total Sales Revenue (Fees Excluded)
#[view]
public fun get_total_sales_revenue(marketplace_addr: address): u64 acquires Marketplace {
    let marketplace = borrow_global<Marketplace>(marketplace_addr);
    let nfts = &marketplace.nfts;
    let nfts_len = vector::length(nfts);
    let total_revenue = 0u64;

    let i = 0;
    while (i < nfts_len) {
        let nft = vector::borrow(nfts, i);
        if (!nft.for_sale && nft.price > 0) {
            total_revenue = total_revenue + nft.price;
        };
        i = i + 1;
    };
    total_revenue
